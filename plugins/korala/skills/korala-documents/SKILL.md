...
- **Korala not signed in:** tell the user to run `codex mcp login korala` in
  a terminal and sign in. They pick what you may do on Korala's consent page.
  Then continue in a new session.
- **No tools:** follow "Without an account". Do not ask for API keys. If the
  user says they connected Korala, the server may still have been starting:
  point them to `required = true` in the Korala docs for Codex
  (https://docs.korala.ai/ai-agents/codex).
